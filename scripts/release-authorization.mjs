...
const topKeys = ["schemaVersion", "enabled", "repository", "authorizationId", "operation", "reconciliationSha256", "nonceSha256", "ownerConfirmationRefSha256", "issuedAt", "expiresAt", "release", "intent", "approvals", "consumption"];
const approvalKeys = contract.authorization.requiredApprovals;
function safeEqual(left, right) { const a = Buffer.from(left ?? ""), b = Buffer.from(right ?? ""); return a.length === b.length && timingSafeEqual(a, b); }

export function verifyAuthorizationReceipt(receipt, manifest, options = {}) {
  exactKeys(receipt, topKeys, "authorization receipt");
  if (receipt.schemaVersion !== 3 || receipt.enabled !== true) throw new Error("formal release authorization is disabled");
  ...
  requireDigest(receipt.nonceSha256, "nonce digest"); requireDigest(receipt.ownerConfirmationRefSha256, "owner confirmation reference digest");
  if (!options.nonce || !safeEqual(sha256Bytes(Buffer.from(options.nonce)), receipt.nonceSha256)) throw new Error("authorization nonce mismatch");
  const issued = Date.parse(receipt.issuedAt), expires = Date.parse(receipt.expiresAt), now = Date.parse(options.now ?? new Date().toISOString());
  if (![issued, expires, now].every(Number.isFinite) || expires <= issued || expires - issued > contract.authorization.maximumLifetimeSeconds * 1000) throw new Error("authorization lifetime is invalid");
  ...
  requireOid(receipt.release.commit, "authorized release commit"); requireOid(receipt.release.tree, "authorized release tree"); requireDigest(receipt.release.manifestSha256, "authorized manifest digest");
  ...
  validateReleaseIdentity({ version: manifest.version, tag: receipt.release.tag, distTag: receipt.intent.distTag });
  if (receipt.intent.registry !== manifest.registry || receipt.intent.registry !== contract.registry || receipt.intent.distTag !== "latest") throw new Error("authorization registry/latest intent drift");
  ...
  for (let index = 0; index < manifest.packages.length; index += 1) {
    const authorized = receipt.intent.packages[index], planned = manifest.packages[index];
    exactKeys(authorized, ["name", "version", "tarball", "sha256", "integrity"], `authorized package ${index}`);
    for (const key of ["name", "version", "tarball", "sha256", "integrity"]) if (authorized[key] !== planned[key]) throw new Error(`authorization package ${key} drift`);
  }
  exactKeys(receipt.approvals, approvalKeys, "authorization approvals");
  for (const gate of approvalKeys) if (receipt.approvals[gate] !== true) throw new Error(`formal release gate is not approved: ${gate}`);
  exactKeys(receipt.consumption, ["state", "tag"], "authorization consumption");
  if (receipt.consumption.state !== "unused" || receipt.consumption.tag !== `npm-release-consumed/${receipt.release.tag}/${receipt.authorizationId}`) throw new Error("authorization receipt is replayed or has invalid consumption identity");
  ...
  verifyReleaseManifest(manifest, { expectedVersion: manifest.version, expectedCommit: receipt.release.commit });
  return receipt;
}
