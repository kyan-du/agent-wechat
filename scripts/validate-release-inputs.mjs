...
const approvedSensitiveInstructionHashes=new Set([
  ...
  'cbbdaa3dffcf2b653070e88bc153900fa3b3e6a3ba28bc9f4039d460f46c862b', // WeChat
...
const w=manifest.sources.wechat;assert.equal(w.version,'4.1.13.23');const wrun=one('RUN WECHAT_VERSION=','WeChat source installation');
const wechatUrls={amd64:'https://web.archive.org/web/20260921152423if_/https://dldir1v6.qq.com/weixin/Universal/Linux/WeChatLinux_x86_64.deb',arm64:'https://web.archive.org/web/20260923032446if_/https://dldir1v6.qq.com/weixin/Universal/Linux/WeChatLinux_arm64.deb'};
const wechatPins={amd64:'b7d0f8d53e9f648bc2c77a6096a04100d008f2d9f0d3988a2a4859b5992aca0a',arm64:'51784a262c725ef1595dd833f456190e913583dd81c24dd8fe587532bc91c0dc'};
const liveWeChatDeb=/^https:\/\/dldir1v6\.qq\.com\/weixin\/Universal\/Linux\/WeChatLinux_(?:x86_64|arm64)\.deb$/;
const waybackIfDeb=/^https:\/\/web\.archive\.org\/web\/\d{14}if_\/https:\/\/dldir1v6\.qq\.com\/weixin\/Universal\/Linux\/WeChatLinux_(?:x86_64|arm64)\.deb$/;
for(const a of ['amd64','arm64']){const v=w.artifacts[a];assert.equal(v.packageArchitecture,a);assert.equal(v.url,wechatUrls[a]);assert.equal(v.sha256,wechatPins[a]);assert.match(v.url,waybackIfDeb);assert.ok(v.url.includes('if_'));assert.ok(!v.url.includes('id_'),`${a}: truncated id_ snapshots are not the pinned payload`);assert.ok(!liveWeChatDeb.test(v.url),`${a}: live Tencent CDN must not be the build URL`);assert.ok(wrun.includes(`WECHAT_${a.toUpperCase()}_URL=${v.url} && WECHAT_${a.toUpperCase()}_SHA256=${v.sha256} && `),`${a}: immutable WeChat constants must exactly match manifest`);assert.ok(wrun.includes(`${a}) WECHAT_URL=`));}
assert.doesNotMatch(wrun,/(?:WECHAT_(?:AMD64|ARM64)_URL|WECHAT_URL)=https:\/\/dldir1v6\.qq\.com\/weixin\/Universal\/Linux\/WeChatLinux_/);
assert.ok(wrun.includes(`http_code=$(curl --retry 3 --max-redirs 0 -o /tmp/wechat.deb -w '%{http_code}' "$WECHAT_URL")`),'WeChat download must not follow redirects');
assert.ok(wrun.includes('test "$http_code" = 200'),'WeChat download must require HTTP 200 before hash checks');
assert.doesNotMatch(wrun,/--location/,'WeChat download must not follow Wayback nearest-capture redirects');
...
assert.ok(downloadWeChat.includes('--max-redirs 0'));
assert.ok(downloadWeChat.includes('[ "$http_code" != "200" ]'));
assert.doesNotMatch(downloadWeChat,/curl[^]*--location/);
...
for(const p of ['docker/release-inputs.json','docker/release-instruction-allowlist.json','docker/release-materials/requirements.lock','docker/release-materials/frida_tools-14.10.4-py3-none-any.whl','scripts/validate-release-inputs.mjs','scripts/test-release-inputs.mjs','scripts/test-download-wechat.mjs','scripts/download-wechat.sh'])assert.ok(workflow.includes(`- '${p}'`),`workflow path missing: ${p}`);
