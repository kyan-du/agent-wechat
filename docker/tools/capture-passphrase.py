...
Production helper. Attach before QR or saved-account login applies the
Data-object passphrase. Never prints key bytes. Writes a mode-0600 file
only after HMAC-SHA512 against contact.db succeeds.
"""
...
def load_extract_keys(path: str | None = None):
    ...
    candidates.append(os.path.join(HERE, "extract-keys.py"))
    candidates.append("/opt/tools/extract-keys.py")
...
def mark_hooks_ready(out_path: str) -> None:
    """Signal that Frida hooks are attached so login can proceed."""
    ready = f"{out_path}.ready"
    fd = os.open(ready, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
    os.close(fd)
...
def attach_and_capture(pid: int, script_src: str, page1: bytes | None, out_path: str,
    ...
    def on_message(message, data):
        ...
        if tag == "ready":
            ...
            log(f"{tag} {extra}")
            mark_hooks_ready(out_path)
            return
        if tag in ("hooked", "data"):
            extra = {k: v for k, v in payload.items() if k != "tag"}
            log(f"{tag} {extra}")
