...
The v2 receipt reuses and extends #49's independently reviewed exact gates: protected annotated authorization ref, exact receipt commit and tag-object OIDs, release commit/tree, regular mode-`100644` strict JSON blob, package/registry/dist-tag intent, nonce/expiry/replay protection, manifest/tarball integrity, and fail-closed npm reconciliation. The one inactive publisher identity is `.github/workflows/npm-agent-release.yml`; it accepts both channels. `.github/workflows/npm-agent-stable.yml` is read-only stable contract validation and cannot act as a second npm publisher.
...
2. Configure exactly one Trusted Publisher per package, all bound to `.github/workflows/npm-agent-release.yml`, after verifying npm's Environment binding supports its separate `npm-prerelease`/`npm-production` jobs. The current workflow grants no reachable OIDC permission; no long-lived token fallback is allowed.
