...
Stable is not a mode switch in authorization, validation, Environment approval, version grammar, dist-tag, or Release semantics. `.github/workflows/npm-agent-stable.yml` currently records that separate input/approval blueprint and future `npm-production` Environment. It accepts only `X.Y.Z`, targets `latest`, creates a non-prerelease GitHub Release, and permanently requires both owner exact confirmation and a non-Agent GitHub Environment approval. However, npm's one-Trusted-Publisher-per-package constraint means this separate workflow cannot also publish the same packages when prerelease uses another workflow. Activation must first refactor the actual OIDC publisher into one npm-trusted workflow while preserving distinct channel validation and Environment gates; until that design receives exact-head review, stable publication remains unreachable.
...
- **npm owner:** bootstrap package ownership if needed. npm supports only one GitHub Actions Trusted Publisher configuration per package, so one package cannot simultaneously trust both `.github/workflows/npm-agent-release.yml` and `.github/workflows/npm-agent-stable.yml`; GitHub Environment separation does not change that npm identity constraint. Before activation, owner/product must choose a supported single-publisher topology (for example one exact publisher workflow with separately gated prerelease/stable reusable release logic), or a later verified npm capability that provides two identities. The current two-workflow blueprints intentionally remain inactive and must not claim stable publishability. No long-lived token fallback is accepted.
