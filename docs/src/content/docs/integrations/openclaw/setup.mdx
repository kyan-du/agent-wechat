...
This plugin's channel id is `agent-wechat`. OpenClaw's official catalog already owns `wechat` / `weixin` / `微信`.
...
```bash
openclaw config set channels.agent-wechat.enabled true
openclaw config set channels.agent-wechat.serverUrl <your-server-url>
openclaw config set channels.agent-wechat.token <your-token>
openclaw plugins inspect agent-wechat --runtime
```
