...
1. **Puppet mode** — import `@kyan-du/agent-wechat-wechaty-puppet` directly and connect to the agent-wechat REST API
...
Create the example below as `bot.mjs` in that consumer directory and run it with `node bot.mjs`. The tarball install makes the package import resolve exactly as it will for a published consumer, without relying on workspace resolution.
...
Gateway mode wraps the puppet in gRPC, so existing Wechaty bots can connect using the standard `wechaty-puppet-service` — no `@kyan-du/agent-wechat-wechaty-puppet` dependency needed. This is the recommended approach for [Managed Hosting](/hosting/managed-hosting) instances, where the gateway is already running.
