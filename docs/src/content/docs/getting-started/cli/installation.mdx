...
Only after P1-B verifies npm publication, install the fork with `npm install -g @kyan-du/agent-wechat-cli`. Until then, follow the repository source installation and invoke `pnpm cli --`. To recover a persisted session rejected for an unsafe image, build the local image and explicitly replace the session selection:
